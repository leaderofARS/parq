...
impl fmt::Display for ProcessingMetrics {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let rows: &[(&str, String)] = &[
            (
                "Input size",
                format!("{:.2} MiB", self.input_bytes as f64 / 1_048_576.0),
            ),
            (
                "Output size",
                format!("{:.2} MiB", self.output_bytes as f64 / 1_048_576.0),
            ),
            ...
            (
                ...
                format!(
                    "{:.2?}",
                    std::time::Duration::from_millis(self.total_duration_ms)
                ),
            ),
            (
                "  ↳ Parse",
                format!(
                    "{:.2?}",
                    std::time::Duration::from_millis(self.parse_duration_ms)
                ),
            ),
            (
                "  ↳ Write",
                format!(
                    "{:.2?}",
                    std::time::Duration::from_millis(self.write_duration_ms)
                ),
            ),
            (
                "Throughput",
                format!("{:.1} MiB/s", self.throughput_mib_per_sec()),
            ),
